...
@implementation selHosListViewController
...
- (NSString *)getSearchURL{
	...
	NSString *serverUrl = [[[NSString stringWithString: [utils performSelector:@selector(getServerURL)]] stringByAppendingString:@"hospital/json&code="] stringByAppendingString:[self.searchBar text]];
	return serverUrl;
}
...
- (void)loadLocalRows{
	@synchronized(self){
		...
		self.totalCount = [dao getTableRowCount:IreferConstraints.hospitalTableName searchValue:self.searchBar.text];
		
		self.dataSource = [dao getTableRowList:IreferConstraints.hospitalTableName searchValue:self.searchBar.text limit:self.currentLimit];
