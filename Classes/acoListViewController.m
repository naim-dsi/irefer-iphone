...
@implementation acoListViewController
...
- (void)viewDidLoad {
	...
	if (self.isSearchFromOnline) {
		...
		NSString *serverUrl = [[NSString stringWithString: [utils performSelector:@selector(getServerURL)]] stringByAppendingFormat:@"aco/json?limit=%d",self.currentLimit];
...
- (NSString *)getSearchURL{
	...
	NSString *serverUrl = [[[NSString stringWithString: [utils performSelector:@selector(getServerURL)]] stringByAppendingString:@"aco/json?code="] stringByAppendingString:[self.searchBar text]];
	return serverUrl;
}
...
- (void)loadLocalRows{
	@synchronized(self){
		...
		self.totalCount = [dao getTableRowCount:IreferConstraints.acoTableName searchValue:self.searchBar.text];
		
		self.dataSource = [dao getTableRowList:IreferConstraints.acoTableName searchValue:self.searchBar.text limit:self.currentLimit];
